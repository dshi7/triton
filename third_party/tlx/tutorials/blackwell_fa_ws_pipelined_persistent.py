...
def _apply_causal_mask(
    qk, col_limit, BLOCK: tl.constexpr, keep_ge: tl.constexpr = False
):
...
def _softmax_inner_loop(
    ...
    for start_n in tl.range(lo, hi, BLOCK_N):
        ...
        if RESCALE_OPT:
            alpha_ = (
                m_i - m_ij
            ) * qk_scale  # alpha_ is 1D distributed over the warp group
    ...
    with tlx.async_tasks():
        ...
        with tlx.async_task(num_warps=4, registers=168, replicate=NUM_MMA_GROUPS):
            ...
            while tile_id != -1:
                ...
                tlx.local_store(
                    l_tiles[cid], tl.join(l_i, l_i) if SCALAR_N == 2 else l_i[:, None]
                )
                tlx.local_store(
                    m_tiles[cid], tl.join(m_i, m_i) if SCALAR_N == 2 else m_i[:, None]
                )
        ...
        with tlx.async_task(num_warps=1, registers=24):
            ...
            while tile_id != -1:
                ...
                for i in tl.range(lo + BLOCK_N, hi, BLOCK_N):
                    ...
                    accum_cnt_kv += 2
                    k_bufIdx, k_phase = get_bufidx_phase(accum_cnt_kv, NUM_BUFFERS_KV)
                    v_bufIdx, v_phase = get_bufidx_phase(
        ...
        with tlx.async_task(num_warps=1, registers=24):
            ...
            while tile_id != -1:
                ...
                tlx.barrier_expect_bytes(
                    q_fulls[q_bufIdx], Q_BYTES_PER_ELEM * BLOCK_M_SPLIT * HEAD_DIM
                )
...
def _bwd_mma_dots_1cta(
    ...
    for j in range(1, num_steps):
        ...
        tmem_buf_id_prev, tmem_phase_prev = get_bufidx_phase(
...
def _bwd_mma_dots_2cta(
    ...
    for j in range(1, num_steps):
        ...
        # before Dot 5's write lands in its storage.
        #
        # Phase (important, do NOT "fix" to `^ 1`): Dot 1 above waited
        # qk_empties on `tmem_phase ^ 1` for the *previous* generation's release
        # (qk(j-1)). Dot 5 must wait for the *next* generation — compute's
        # release of qk(j). qk_empties is single-buffered (one barrier, NUM_
        # BUFFERS_TMEM slot), so consecutive compute releases toggle the phase;
        # the generation after Dot 1's is therefore `tmem_phase`, not
        # `tmem_phase ^ 1`. Using `^ 1` here re-waits the generation Dot 1
        # already consumed → the barrier is already at that phase → the wait
        # returns immediately (a no-op), leaving Dot 5 ordered against compute's
        # qk(j) read only by MMA-pipe timing (the latent cross-partition WAR).
...
def _bwd_load_1cta(
    ...
    for _ in range(1, num_steps):
        ...
        tlx.barrier_wait(q_empties[q_buf_id], q_phase ^ 1)
        tlx.barrier_expect_bytes(
            q_fulls[q_buf_id], Q_BYTES_PER_ELEM * BLOCK_M1 * (HEAD_DIM // NUM_CTAS)
        )
        tlx.async_descriptor_load(
            desc_q,
            q_tiles[q_buf_id],
        ...
        tlx.barrier_wait(do_empties[do_buf_id], do_phase ^ 1)
        tlx.barrier_expect_bytes(
            do_fulls[do_buf_id], DO_BYTES_PER_ELEM * BLOCK_M1 * (HEAD_DIM // NUM_CTAS)
        )
        tlx.async_descriptor_load(
            desc_do,
            do_tiles[do_buf_id],
        ...
        if USE_2CTA:
            ...
            if cluster_cta_rank == 0:
                own_tmem = tlx.local_slice(
                    dsT_tmem_tiles[ds_buf_id], [0, 0], [BLOCK_N1, BLOCK_M1 // NUM_CTAS]
                )
                ...
                own_smem = tlx.local_slice(
                    ds_tiles[ds_buf_id], [0, 0], [BLOCK_N1, BLOCK_M1 // NUM_CTAS]
                )
            else:
                own_tmem = tlx.local_slice(
                    dsT_tmem_tiles[ds_buf_id],
                ...
                own_smem = tlx.local_slice(
                    ds_tiles[ds_buf_id], [BLOCK_N1, 0], [BLOCK_N1, BLOCK_M1 // NUM_CTAS]
                )
            ...
            remote_dst = own_smem
            tlx.barrier_expect_bytes(
                ds_peer_fulls[ds_buf_id], 2 * BLOCK_N1 * (BLOCK_M1 // NUM_CTAS)
            )
...
def _attn_bwd_ws(
    ...
    REUSE_DP_FOR_DQ: tl.constexpr = (
        (BLOCK_M1 == 128) and (HEAD_DIM == 128) and (NUM_CTAS == 1)
    )
    ...
    if USE_WARP_BARRIER:
        ...
        dv_empties = tlx.alloc_barriers(
            num_barriers=NUM_BUFFERS_KV, arrive_count=NUM_CTAS
        )
    ...
    sdv_store_buf = tlx.local_alloc(
        (BLOCK_N1, DKV_STORE_NCOL), tlx.dtype_of(desc_dv), NUM_BUFFERS_KV, reuse=v_tiles
    )
    sdk_store_buf = tlx.local_alloc(
        (BLOCK_N1, DKV_STORE_NCOL), tlx.dtype_of(desc_dk), NUM_BUFFERS_KV, reuse=k_tiles
    )
        ...
        if USE_2CTA:
            with tlx.async_task(num_warps=1, registers=40):
                for blk_idx_relay in range(num_steps):
                    ds_buf_id_relay, ds_phase_relay = get_bufidx_phase(
                        blk_idx_relay, NUM_BUFFERS_DS
                    )
...
class _attention(torch.autograd.Function):
    ...
    def backward(ctx, do):
        ...
        dq_accum = torch.zeros(
            [BATCH, N_HEAD, N_CTX, ctx.HEAD_DIM], device=q.device, dtype=torch.float32
        )
        PRE_BLOCK = 128
        BLK_SLICE_FACTOR = 2
        ...
        desc_kt = TensorDescriptor(
            arg_k, shape=desc_shape, strides=desc_strides, block_shape=dummy_block
        )
        desc_qt = TensorDescriptor(
            q, shape=desc_shape, strides=desc_strides, block_shape=dummy_block
        )
        desc_dot = TensorDescriptor(
            do, shape=desc_shape, strides=desc_strides, block_shape=dummy_block
        )
