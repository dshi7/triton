...
| `canJoinReuseGroupChain(owner, cand)` | `:3821` | **N-way sibling formation.** Lets `cand` join a group with **no pairwise dep** when the whole group (≥3) is a unique dependency chain under the *sound* predicate. Used by the enumeration and the post-pass. |
