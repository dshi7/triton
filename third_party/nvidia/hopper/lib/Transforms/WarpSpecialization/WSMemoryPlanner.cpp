...
public:
  ...
  /// chain order under the SOUND edge policy (crossPartitionProgOrder=false) --
  /// the same predicate code partitioning uses to place the reuse barrier, but
  /// stricter, so anything accepted here is also orderable there. Chain order is
  /// a happens-before, so the members are not co-live and time-multiplexing the
  /// shared slot is safe. Requires >=3 members (pairwise is handled by
  /// hasPotentialReuse) and that candidate fits the owner's columns.
  ///
  /// Only the (opt-in) enumeration path calls this; the default first-fit path
  /// never does, so default compiles are unaffected.
  ...
  FailureOr<unsigned> allocateTMemAllocs2(
    ...
    if (topK > 1 || pick > 0) {
      ...
      if (haveFirstFit && !sols.empty()) {
        ...
        // Pin the first-fit packing to rank 0.
        uint64_t ffSig = tmemStateSignature(allocs, firstFit);
        bool ffFound = false;
        for (unsigned i = 0; i < sols.size(); ++i) {
          if (sols[i].sig == ffSig) {
            std::rotate(sols.begin(), sols.begin() + i, sols.begin() + i + 1);
            ffFound = true;
            break;
          }
        }
